...
class PHP_CRUD_API {
	...
	public function executeCommand() {
		...
		if (strcasecmp($this->settings['request'], 'getData') == 0) {
			$sql = "SELECT FLOOR(fecha/3600)*3600 as fecha, device_id, device_name, ROUND(avg(temperatura),1) as temperatura, ROUND(avg(humedad),1) as humedad, ROUND(avg(luz),1) as luz FROM data GROUP BY FLOOR(fecha/3600)*3600";
			$result = $this->db->query($sql);
			$json = mysqli_fetch_all($result, MYSQLI_ASSOC);
			echo json_encode($json);
		} 
		else if (strcasecmp($this->settings['request'], 'updateTemperatura') == 0) {
			$valores = json_decode($_POST['data'], true);

			$temperatura_min = $valores['temperatura_min'];
			$temperatura_max = $valores['temperatura_max'];

			$sql = "UPDATE config SET valor = $temperatura_min WHERE clave = 'temperatura_min'";
			$this->db->query($sql);
			$sql = "UPDATE config SET valor = $temperatura_max WHERE clave = 'temperatura_max'";
			$this->db->query($sql);
			//$json = mysqli_fetch_all($result, MYSQLI_ASSOC);
			//echo json_encode($json);
		} 
...
// $auth = new PHP_API_AUTH(array(
// 	'authenticator'=>function($user,$pass){ $_SESSION['user']=($user=='admin' && $pass=='Admin12345'); }
// ));
// if ($auth->executeCommand()) exit(0);
// if (empty($_SESSION['user']) || !$auth->hasValidCsrfToken()) {
// 	header('HTTP/1.0 401 Unauthorized');
